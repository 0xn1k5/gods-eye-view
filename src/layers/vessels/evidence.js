...
export function createEvidence({ vesselState, services, parts: components, layer, options }) {
...
function _setFocusEvidenceVessels(rows = []) {
  ...
  components.store.reconcileVessels(state.viewer, Array.isArray(rows) ? rows : []);
