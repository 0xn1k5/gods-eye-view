...
import { AIS_FIRST_CONNECT_GRACE_MS, AIS_FIRST_CONNECT_LABEL, AIS_HEALTHY_STATUSES } from './policy.js';

export function createLifecycle({ vesselState, services, parts: components, layer, options }) {
...
function scheduleFirstConnectExpiry(sessionId, delayMs) {
  state.firstConnectTimer = vesselState._aisRuntime.setTimeout(() => {
    ...
    const remainingMs = state.firstConnectDeadline - vesselState._aisRuntime.now();
    if (remainingMs > 0) {
...
const methods = {
  ...
  init(viewer) {
    if (typeof vesselState._source?.getSnapshot !== 'function') throw new TypeError('A snapshot source is required');
    state.viewer = viewer;
    components.rendering.ensureCollections(viewer);
    vesselState._vesselOverlayHost.setVisible(VESSEL_OVERLAY_SOURCE_ID, false);
...
return { clearFirstConnectTimer, invalidateAisSession, beginAisSession, scheduleFirstConnectExpiry, settleFirstConnectPhase, isGraceEligibleTransport, isDefinitiveTransportFailure, markAisUnavailable, resetState, methods };
}
