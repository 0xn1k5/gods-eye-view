...
import { TRAIL_HEIGHT_M, VESSEL_LIFT_M, TRAIL_COLOR, TRAIL_MAX_POINTS, TRAIL_MIN_MOVE_M } from './policy.js';

export function createTracking({ vesselState, services, parts: components, layer, options }) {
...
function vesselTrailPosition(record) {
  ...
  const heightM = components.queries.vesselDatumHeightM(currentGeoidN(record.lat, record.lon), TRAIL_HEIGHT_M);
...
function refloorVesselRecords() {
  ...
  for (const record of state.vesselRecords) {
    ...
    const heightM = components.queries.vesselDatumHeightM(currentGeoidN(record.lat, record.lon), VESSEL_LIFT_M);
...
async function backfillVesselTrail(mmsi, token, reference = mmsi) {
  ...
  const signal = owner ? AbortSignal.any([owner.signal, AbortSignal.timeout(8000)]) : AbortSignal.timeout(8000);
  let samples = null;
  try {
  ...
  if (signal.aborted || owner !== state.trailAbort || !samples || token !== state.trailBackfillToken) return;
  ...
  for (const sample of samples) {
    ...
    const heightM = components.queries.vesselDatumHeightM(currentGeoidN(lat, lon), TRAIL_HEIGHT_M);
...
return { currentGeoidN, vesselTrailPosition, refloorVesselRecords, startSelectedVesselTrail, backfillVesselTrail, appendSelectedVesselTrailFix, clearSelectedVesselTrail, destroySelectedVesselTrail };
}
