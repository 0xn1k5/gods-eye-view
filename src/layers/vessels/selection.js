...
export function createSelection({ vesselState, services, parts: components, layer, options }) {
const { state } = vesselState;
const { resolvePickId, isOwnedByOtherLayer } = services.picking;
const { requestWorldFocus } = services.worldFocus;
const { selectEntityContext, registerEntityContext, clearSelectedEntityContextForLayer } = services.context;
...
function bindVesselInteraction(viewer, handler, keyTarget) {
  ...
  handler.setInputAction((click) => {
    ...
    const cardHit = !record
      ? vesselState._vesselOverlayHost.hitTest?.(click.position?.x, click.position?.y, {
...
return { installInteraction, bindVesselInteraction, selectAndFocusVessel, removeVesselInteraction, onVesselKeyDown, selectVessel, registerSelectedContext, clearSelection, clearVesselInspection };
}
