...
import { vesselTypeCss, vesselOverlayCohortLimit, VESSEL_OVERLAY_SOURCE_ID, applyVesselOverlayPolicy, VESSEL_CARD_FADE_DISTANCE_M } from '../../data/vesselLabels.js';
import { cameraPoseSignature, screenProjectedRotation } from '../../data/iconOrientation.js';
import { DEFAULT_RENDER_ROWS, DEFAULT_ACTIVE_LABELS, VISIBILITY_UPDATE_MS, FOCUS_UPDATE_MS, LABEL_GRID_PX, CARD_MIN_SEP_PX } from './policy.js';

export function createRendering({ vesselState, services, parts: components, layer, options }) {
const { state } = vesselState;
const { registerSpriteCollection } = services.sprites;
const { forgetSpriteFocus, focusNowMs, getFocusTarget, focusPassIsNeeded, advanceSpriteFocus, focusAlphaNeedsWrite } = services.focus;
...
function shipIcon(record, selected) {
  ...
  if (vesselState.shipIconCache.has(key)) return vesselState.shipIconCache.get(key);
  ...
  if (focusPass && scene && camera) {
    const result = applyVesselFocusDeemphasis({
      ...
      screenPositionFor: (position) => (
        Cesium.SceneTransforms.worldToWindowCoordinates(scene, position, vesselState._scratchFocusScreen)
      ),
...
function updateClusteredLabels(records) {
  ...
  const entries = selected ? [components.cards.buildSelectedVesselCard(selected)] : [];
  ...
  for (const candidate of ranked) {
    ...
    if (!components.cards.cardScreenSeparated(accepted, candidate, CARD_MIN_SEP_PX)) continue;
...
return { renderRowLimit, labelRowLimit, ensureCollections, addRecordPrimitives, removeRecordPrimitives, shipScale, vesselCourseDeg, shipIcon, installRuntime, updateVisibility, applyVesselFocusDeemphasis, makeOccluder, isVisible, updateClusteredLabels, publishVesselOverlayEntries, labelPriority, hasUsefulName, setVisible };
}
