...
export function createIngestion({ vesselState, services, parts: components, layer, options }) {
...
async function loadLivePositions(viewer) {
  ...
  try {
    ...
    const snapshot = await vesselState._source.getSnapshot({ maxRows: components.rendering.renderRowLimit() }, { signal });
    ...
    applyAisFeedSnapshot(viewer, {
      rows: snapshot.records.map(vesselDisplayRow), observedAtMs: snapshot.observedAtMs, freshness: snapshot.freshness, complete: snapshot.complete, rawRowCount: snapshot.rawRowCount, reason: snapshot.reason, status: snapshot.transportStatus,
    ...
    if (ownsAisRequest(requestController, requestSessionId) && error?.name !== 'AbortError') {
      components.lifecycle.markAisUnavailable(error?.message || 'AIS live load failed');
...
function applyAisFeedSnapshot(viewer, payload) {
  ...
  if (snapshot.acceptedRowCount === 0) {
    ...
    if (components.lifecycle.isDefinitiveTransportFailure(snapshot.transportStatus)) {
  ...
  components.store.reconcileVessels(viewer, snapshot.acceptedRows, { complete: payload?.complete !== false });
  state.count = state.vesselRecords.length;
  state.stale = Boolean(payload?.refreshing) || payload?.complete === false || payload?.freshness === 'unknown';
  ...
  state.lastUpdate = Object.hasOwn(payload, 'observedAtMs') ? payload.observedAtMs : vesselState._aisRuntime.now();
...
return { loadLivePositions, ownsAisRequest, applyAisFeedSnapshot, vesselDisplayRow, methods };
}
