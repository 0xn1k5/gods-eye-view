...
export function createStore({ vesselState, services, parts: components, layer, options }) {
...
function reconcileVessels(viewer, rows, { complete = true } = {}) {
  ...
  for (const [mmsi, record] of state.vesselMap) {
    ...
    if (!complete && Number.isFinite(record.receivedAtMs) && receivedAtMs - record.receivedAtMs < PARTIAL_RETENTION_MS) {
    ...
    if (state.trailMmsi === mmsi) components.tracking.clearSelectedVesselTrail();
  }
...
function updateRecordInPlace(record, next) {
  ...
  if (record.billboard) {
    ...
    record.billboard.scale = components.rendering.shipScale(record) * (selected ? 1.2 : 1);
...
function normalizeVessel(row) {
  ...
  const heightM = components.queries.vesselDatumHeightM(components.tracking.currentGeoidN(lat, lon), VESSEL_LIFT_M);
...
return { reconcileVessels, updateRecordInPlace, normalizeVessel, finiteNumber };
}
