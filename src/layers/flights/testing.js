...
export function createTesting({ flightState, services, parts, layer, resolveAsset }) {
...
function _floorGroundedDisplayPositionForTest(
  ...
  return parts.motion._floorGroundedDisplayPosition(icao24, info, pos, modelOwnsVisual, nowMs);
}
...
function _addFlightTrackingCandidateForTest({ icao24, meta, billboard, history = [] }) {
...
function _armFlightTrackingRestoreForTest(id, origin = 'share-restore') {
  flightState._pendingTrackingRestore = id === null
    ? null
...
function _trackedBillboardColorForTest() {
  return parts.rendering._modelOwnsVisual(flightState._trackedIcao) ? CYAN_TRANSPARENT : Cesium.Color.CYAN;
}
...
return { _floorGroundedDisplayPositionForTest, _clearDisplayFloorStateForTest, _setTrackedFlightRefreshStateForTest, _setFlightTrackingRefreshOutcomeForTest, _addFlightTrackingCandidateForTest, _militaryLayerSuppressesForTest, _armFlightTrackingRestoreForTest, _pendingFlightTrackingRestoreForTest, _applyPendingFlightTrackingRestoreForTest, _setCockpitDetectionSubjectForTest, _trackedModelRegimeActiveForTest, _updateTrackedModelForTest, _trackedBillboardColorForTest, _driveFleetModelHandoffForTest, _ensureFleetModelForTest };
}
