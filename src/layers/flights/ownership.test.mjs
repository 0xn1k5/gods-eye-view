...
test('flight state owns distinct mutable records, scratch objects and floor samplers', () => {
  ...
  for (const key of ['_flightData', '_billboards', '_positionHistory', '_displayFloorState',
    '_groundSnap', '_scratchCarto', '_models', '_activeUpdateControllers', 'lifetime']) {
...
test('aborted enrichment cannot write into a later lifecycle or decrement its active count', async () => {
  let resolve;
  let signal;
  const source = { getEnrichment(query, options) {
  ...
  let writes = 0;
  enrichment._enqueueEnrich('test', { kind: 'type', id: 'abc123' }, () => writes++);
...
test('a synchronous enrichment failure stays bounded and releases its slot', async () => {
  const flightState = createFlightState({ services: services(), source: {
    getEnrichment() { throw new Error('fixture failure'); },
  } });
  const enrichment = createEnrichment({ flightState, services: {}, parts: {} });
  assert.doesNotThrow(() => enrichment._enqueueEnrich('test', {}, () => assert.fail()));
  await new Promise((done) => setImmediate(done));
  assert.equal(flightState._enrichActive, 0);
});
