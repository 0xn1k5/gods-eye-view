...
export function createLifecycle({ flightState, services, parts, layer, resolveAsset }) {
const { clearFocusTarget } = services.focus;
const { registerSpriteCollection, restoreSpriteOrder, restoreSpriteOrderOnEnable } = services.sprites;
...
const methods = {
  ...
  setSource(source) {
    if (flightState._viewer) throw new Error('Configure the source before layer initialization');
   ...
   */
  init(viewer) {
    if (flightState._viewer) throw new Error('Flight layer is already initialized');
    if (typeof flightState._source?.getSnapshot !== 'function') throw new TypeError('A snapshot source is required');
    if (flightState.lifetime.signal.aborted) flightState.lifetime = new AbortController();
    ...
    if (!flightState._preloadModel) {
      ...
      Cesium.Model.fromGltfAsync({ url: resolveAsset(PLANE_MODEL_URL), asynchronous: false })
        .then((m) => {
          if (epoch === flightState._modelEpoch) { flightState._preloadModel = m; flightState._planeModelLoaded = true; }
    ...
    flightState._cockpitContactMode = document.body.classList.contains('cockpit-mode');
    flightState._cockpitNearContacts = new Set();
    if (!flightState._cockpitModeListener) {
      flightState._cockpitModeListener = (event) => parts.tracking._applyCockpitState(event?.detail);
      window.addEventListener('gev:cockpit-mode-changed', flightState._cockpitModeListener);
    }
    // Fresh session — full bucket, anchor re-seeded on the first sweep.
    flightState._enrichAmbientBudget = parts.enrichment._ambientBudgetKnobs().ceil;
    ...
    if (!flightState._milActiveChangeUnsub) {
      flightState._milActiveChangeUnsub = onMilitaryLayerActiveChange(parts.tracking._onMilitaryActiveChange);
    }
   ...
   */
  enable(viewer) {
    if (flightState._billboardCollection) flightState._billboardCollection.show = true;
    ...
    parts.tracking._setCockpitContactMode(document.body.classList.contains('cockpit-mode'));
    ...
    if (!flightState._geoidReady) {
      ...
      ensureGeoidReady()
        .then(() => { if (!lifetime.signal.aborted) flightState._geoidReady = true; })
        .catch(() => { /* geoid grid failed to load — baro path stays un-geoid-corrected until retried */ });
    }
    parts.tracking._installClickHandler(viewer);
    registerPickOwner('flights', (pickedId) => flightState._billboards.has(pickedId));
    ...
    if (!flightState._preRenderRemove && viewer?.scene) {
      flightState._preRenderRemove = viewer.scene.preRender.addEventListener(parts.rendering._fleetTick);
    }
    if (!flightState._trackedModelPreUpdateRemove && viewer?.scene) {
      flightState._trackedModelPreUpdateRemove = viewer.scene.preUpdate.addEventListener(parts.rendering._updateTrackedModel);
    }
  ...
  disable(viewer) {
    ...
    if (flightState._billboardCollection) flightState._billboardCollection.show = false;
    releaseContinuousRender('flights');
    parts.rendering._releaseModels();
    if (flightState._modelCollection) flightState._modelCollection.show = false;
  ...
  destroy(viewer) {
    ...
    document.removeEventListener('keydown', parts.tracking._onKeyDown);
    if (flightState._cockpitModeListener) {
      window.removeEventListener('gev:cockpit-mode-changed', flightState._cockpitModeListener);
    ...
    if (flightState._preloadModel) { try { flightState._preloadModel.destroy(); } catch { /* gone */ } flightState._preloadModel = null; }
    ...
    if (flightState._enrichDripTimer) { clearTimeout(flightState._enrichDripTimer); flightState._enrichDripTimer = null; }
