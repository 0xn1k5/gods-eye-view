...
import { geoidSurfaceLastResortM, pickRenderAltitudeM } from '../../data/renderAltitude.js';
import { classifyAircraft } from '../../data/aircraftClass.js';
import { turnRateFromFixHistory, liftRepeatedGroundFix, synthesizeForwardKinematicsFix } from '../../data/motionModel.js';
import { aircraftIcon } from '../../data/aircraftIcons.js';
import { GROUND_FLOOR_CLAMP_RADIUS_KM, GROUND_FLOOR_WARM_MAX_ALT_M, POSITION_HISTORY_LIMIT, LANDED_MISSING_POLL_LIMIT, MISSING_POLL_LIMIT, ERROR_BACKOFF_INTERVAL } from './policy.js';

export function createIngestion({ flightState, services, parts, layer, resolveAsset }) {
const { refreshMilitaryRegistryIfStale, isMilitaryIcao } = services.militaryRegistry;
const { geoidHeight } = services.geoid;
const { cachedGroundFloor, floorAltitudeM, warmGroundFloor } = services.groundFloor;
...
const methods = {
  ...
  async update(viewer, { signal = null } = {}) {
    ...
    try {
      ...
      const snapshot = await flightState._source.getSnapshot(parts.controller._flightQuery(viewer || flightState._viewer), { signal: updateSignal });
      updateSignal.throwIfAborted();
      ...
      const viewerCarto = (viewer || flightState._viewer)?.camera?.positionCartographic || null;
      ...
      for (const observation of usableStates) {
        ...
        if (onGround) {
          ...
          if (viewerLatDeg != null &&
              parts.queries._approxDistanceKm(viewerLatDeg, viewerLonDeg, lat, lon) <= GROUND_FLOOR_CLAMP_RADIUS_KM) {
        ...
        if (!newest || Cesium.JulianDate.greaterThan(fixTime, newest.time)) {
          ...
          const modelOwnsGroundVisual = parts.rendering._modelOwnsVisual(icao24);
          ...
          if (kinematicsChanged) {
            ...
            if (synthetic) {
              ...
              meta.turnRateDps = turnRateFromFixHistory(history);
              if (isTracked) parts.tracking._appendTrailFix(synthetic.position.clone());
            }
          }
        }
        ...
        if (flightState._billboards.has(icao24)) {
          ...
          if (!isTracked) {
            ...
            bb.color = parts.rendering._fleetBillboardColor(icao24).withAlpha(bb.color?.alpha ?? 1);
          }
          if (prevMeta?.klass !== meta.klass || groundFlipped || flightState._cockpitContactMode) {
          ...
          if (prevMeta?.klass !== meta.klass) parts.rendering._syncModelToClass(icao24);
        } else {
          const bb = flightState._billboardCollection.add({
            position,
            image: aircraftIcon(parts.rendering._iconKind(icao24, meta.klass)),
            ...
            color: isTracked ? Cesium.Color.CYAN : parts.rendering._fleetBillboardColor(icao24),
            sizeInMeters: false,
        ...
        if (isTracked && groundFlipped && !meta.onGround) parts.tracking._startTrail(icao24);
      ...
      for (const [icao24, bb] of flightState._billboards) {
        ...
        if (!snapshot.complete && Date.now() - (flightState._flightData.get(icao24)?.observedReceiptMs ?? 0) < 300000) continue;
        const misses = (flightState._missingPolls.get(icao24) || 0) + 1;
        const limit = parts.queries._likelyLanded(icao24) ? LANDED_MISSING_POLL_LIMIT : MISSING_POLL_LIMIT;
        if (misses < limit) {
      ...
      parts.motion._collectDisplayCorridorCells(floorWarmPoints, viewerLatDeg, viewerLonDeg);
      ...
      sampleMeshFloorCells(flightState._viewer?.scene, floorWarmPoints, {
        excludeObjects: [...flightState._billboards.values(), ...flightState._models.values(), flightState._trackedModel].filter(Boolean),
      ...
      flightState._backoff = true;
      flightState._retryAt = Date.now() + (e?.retryAfterMs ?? ERROR_BACKOFF_INTERVAL);
      flightState._lastStatus = e?.status ?? null;
      if (e?.source) { flightState._lastSource = e.source; this.source = flightState._lastSource; }
      flightState._lastError = e?.name === 'LiveSourceError' ? e.message : 'Live data unavailable';
    } finally {
