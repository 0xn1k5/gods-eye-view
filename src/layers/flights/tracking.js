...
import { TRACKED_MODEL_MAX_LOAD_FAILS, TRACKED_MODEL_RETRY_BACKOFF_MS, TRAIL_MAX_POINTS, RENDER_DELAY_SEC, TRAIL_COLOR, CYAN_TRANSPARENT } from './policy.js';

export function createTracking({ flightState, services, parts, layer, resolveAsset }) {
const { selectTrackedSubjectContext, clearTrackedSubjectContext, refreshTrackedSubjectContext } = services.context;
const { isTr3b, tr3bTypeLabel, tr3bConvertedIds } = services.aircraftPresentation;
const { createTrail } = services.trails;
const { ensureGeoidReady, geoidHeight } = services.geoid;
const { resolveGroundFloorCellsBounded, floorAltitudeM, cachedGroundFloor } = services.groundFloor;
...
function _contextSubjectMetadata(icao24) {
  ...
  return {
    ...
    label: parts.queries._contactLabel(icao24, flightState._flightData.get(icao24)),
    ...
    properties: {
      name: parts.queries._contactLabel(icao24, flightState._flightData.get(icao24)),
      ...
      polyline: {
        positions: new Cesium.CallbackProperty(() => {
          ...
          const from = trailHeadStart(
            start, head, parts.motion._trackedModelCenterWorld(), parts.motion._trackedModelEnvelopeM(), flightState._scratchTrailHead,
          );
          if (!from) return [];
...
async function _backfillTrail(icao24, token, oldestFixEpochSec) {
  ...
  if (!path || token !== flightState._trailBackfillToken || icao24 !== flightState._trackedIcao) return;
...
function _clearTracking(skipViewerUntrack = false, {
  ...
  if (flightState._billboards.has(flightState._trackedIcao)) {
    ...
    bb.scale = parts.rendering._fleetBillboardScale(flightState._trackedIcao, flightState._flightData.get(flightState._trackedIcao)?.klass);
...
function _routeIsPlausible(icao24, route) {
  ...
  const carto = Cesium.Cartographic.fromCartesian(bb.position, Cesium.Ellipsoid.WGS84, flightState._scratchCarto);
...
function _trackFlight(icao24, { origin = 'programmatic' } = {}) {
  ...
  flightState._trackedEntity = flightState._viewer.entities.add({
    ...
    billboard: {
      ...
      width: 28,
      height: 28,
      scale: CLASS_SCALE_2D[flightState._flightData.get(flightState._trackedIcao)?.klass] || 1,
      ...
      color: new Cesium.CallbackProperty(() => (
        parts.rendering._modelOwnsVisual(flightState._trackedIcao) ? CYAN_TRANSPARENT : Cesium.Color.CYAN
      ), false),
      sizeInMeters: false,
  ...
  flightState._trackedEntity.gevLabelModel = trackedLabelModelFromText(_trackedLabelText(icao24), '#39d0ff');
  ...
  console.log(`[Data:Flights] Tracking ${parts.queries._contactLabel(icao24, info)} (${icao24})`);
}
...
function _onMilitaryActiveChange(active) {
  ...
  if (active) {
    for (const [icao24, bb] of flightState._billboards) {
      if (!isMilitaryIcao(icao24) || icao24 === flightState._trackedIcao) continue;
...
return { _emitAwarenessEvent, _publishTrackedSelection, _contextSubjectMetadata, _isExplicitTrackingOrigin, _refreshCockpitNearContacts, _setCockpitContactMode, _applyCockpitState, _resetTrackedSelectionState, _trackedModelLoadAllowed, _noteTrackedModelLoadFailure, _trackedModelRegimeActive, _syncTracked2dRotation, _appendTrailFix, _refreshTrailDisplay, _startTrail, _backfillTrail, _clearTrail, _destroyTrail, _clearTracking, _militaryLayerSuppresses, _applyPendingTrackingRestore, _cancelPendingTrackingRestore, _trackedLabelText, _updateTrackedLabelModel, _syncTrackedBillboardImage, _refreshTr3bContact, _refreshTr3bForStyle, _routeIsPlausible, _trackFlight, _onMilitaryActiveChange, _onKeyDown, _installClickHandler };
}
