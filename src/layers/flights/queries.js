...
import { rankContactMatch, contactMatchWins, CONTACT_MATCH_TIER } from '../../data/contactMatch.js';
import { aircraftIncludedInNearby } from '../../data/aircraftNearbyPolicy.js';
import { LANDED_ALT_MAX_M, LANDED_SPEED_MAX_MPS, FOCUS_EVIDENCE_DEV, FLEET_DR_INTERVAL_MS } from './policy.js';

export function createQueries({ flightState, services, parts, layer, resolveAsset }) {
...
const { setFocusDeemphasisParams, getFocusDeemphasisParams, setFocusEvidenceNowMs, advanceFocusEvidenceNowMs } = services.focus;
const { setAircraftRecessionParams, getAircraftRecessionParams } = services.recession;
...
function _describeFlight(icao24) {
  ...
  const basePos = parts.motion._deadReckon(icao24) || (bb ? bb.position : null);
  ...
  const carto = Cesium.Cartographic.fromCartesian(basePos, Cesium.Ellipsoid.WGS84, flightState._scratchCarto);
  ...
  return {
    ...
    stale: Boolean(flightState._missingPolls.get(icao24) || flightState._backoff),
    ...
    origin: info?.route && parts.tracking._routeIsPlausible(icao24, info.route) ? info.route.origin.code : null,
    destination: info?.route && parts.tracking._routeIsPlausible(icao24, info.route) ? info.route.destination.code : null,
    route: info?.route && parts.tracking._routeIsPlausible(icao24, info.route) ? {
...
function mapAnalystRecord(icao24, info, { military = false, routeOk = false } = {}) {
...
const methods = { mapAnalystRecord, 
  ...
  _clearDisplayFloorStateForTest: parts.testing._clearDisplayFloorStateForTest,
  ...
  setParams(params = {}, { origin = 'programmatic' } = {}) {
    ...
    if (typeof params.models3d === 'boolean' && params.models3d !== flightState._models3dEnabled) {
      ...
      if (!flightState._models3dEnabled) {
        ...
        if (flightState._billboardCollection) for (const [icao, bb] of flightState._billboards) { if (icao !== flightState._trackedIcao) bb.show = true; }
      }
    }
    if ((params.models3dMode === 'proximity' || params.models3dMode === 'all') && params.models3dMode !== flightState._models3dMode) {
    ...
    if (typeof params.irBoost === 'boolean' && params.irBoost !== flightState._irBoost) {
    ...
    if (Object.hasOwn(params, 'selectedFlightsTrackingId')) {
      ...
      if (requested === flightState._trackedIcao) {
        ...
        parts.tracking._cancelPendingTrackingRestore();
        if (flightState._trackedIcao) parts.tracking._clearTracking(false, { origin });
      } else {
        const generation = ++flightState._trackingIntentGeneration;
        flightState._pendingTrackingRestore = { id: requested, generation, origin };
        if (flightState._trackedIcao) parts.tracking._clearTracking(false, { origin });
   ...
   */
  refreshTr3b(icao24) { return parts.tracking._refreshTr3bContact(icao24); },
  ...
  getDetectableObjects(options = {}) {
    if (!flightState._billboardCollection || !flightState._billboardCollection.show) return [];
    ...
    const stride = Math.max(1, Math.ceil(flightState._billboards.size / maxCount));
    ...
    for (const [icao24, bb] of flightState._billboards) {
      ...
      if (flightState._cockpitContactMode && icao24.toLowerCase() === flightState._cockpitSubjectId) continue;
      ...
      const spec = modelOwnsVisual ? parts.rendering._modelSpec(info?.klass) : null;
      const pos = isTracked
    ...
    if (flightState._detectionObjects.size > flightState._billboards.size + 512) {
      for (const icao24 of flightState._detectionObjects.keys()) {
        if (!flightState._billboards.has(icao24)) flightState._detectionObjects.delete(icao24);
      }
    }
    return result;
  },
   ...
   */
  findByQuery(query) {
    if (!flightState._flightData || flightState._flightData.size === 0) return null;
  ...
  getNearby(center, range, maxCount = 50, { includeHidden = false } = {}) {
    if (!center || !flightState._billboardCollection || !flightState._billboardCollection.show) return [];
    ...
    for (const [icao24, bb] of flightState._billboards) {
      ...
      const trackedPos = isTracked ? parts.motion._trackedDisplayCached() : null; // cached, no recompute (anti-jitter)
   ...
   */
  hasContact(icao24) {
    if (!flightState._billboardCollection || !flightState._billboardCollection.show || flightState._billboards.size === 0) return null;
    if (!icao24) return false;
    const id = String(icao24).trim();
    return flightState._billboards.has(id) || flightState._billboards.has(id.toLowerCase());
  },


  getAllPositions(maxCount = 500) {
    if (!flightState._billboardCollection || flightState._billboards.size === 0) return [];
    ...
    for (const [icao24, bb] of flightState._billboards) {
      ...
      if (!pos) continue;
      const carto = Cesium.Cartographic.fromCartesian(pos, Cesium.Ellipsoid.WGS84, flightState._scratchCarto);
      if (!carto) continue;
      const info = flightState._flightData.get(icao24);
      result.push({
  ...
  getAnalystRecords(maxCount = 2000) {
    if (!flightState._billboardCollection || !flightState._billboardCollection.show || flightState._flightData.size === 0) return [];
    const limit = Number.isFinite(maxCount) ? Math.max(1, Math.floor(maxCount)) : 2000;
    const result = [];
    for (const [icao24, info] of flightState._flightData) {
      const routeOk = !!info?.route && parts.tracking._routeIsPlausible(icao24, info.route);
  ...
  trackById(icao24, { origin = 'programmatic' } = {}) {
    ...
    if (parts.tracking._isExplicitTrackingOrigin(origin)) parts.tracking._cancelPendingTrackingRestore();
    if (flightState._trackedIcao === id) return parts.tracking._publishTrackedSelection(id, origin);
  ...
  refocusTrackedById(icao24, { origin = 'programmatic' } = {}) {
    if (!icao24 || flightState._cockpitContactMode || !flightState._viewer || !flightState._trackedEntity) return false;
    let id = String(icao24).trim();
    if (!flightState._billboards.has(id)) id = id.toLowerCase();
    if (
  ...
  ...(FOCUS_EVIDENCE_DEV ? {
    __focusEvidence: Object.freeze({
      ...
      takeFrameClock(startMs = 1_000_000_000) {
        if (!flightState._viewer || !Number.isFinite(startMs)) return { ok: false, nowMs: null };
        ...
        return new Promise((resolve) => setTimeout(() => {
          if (flightState._viewer?._cesiumWidget) flightState._viewer._cesiumWidget._renderLoopRunning = false;
      ...
      releaseFrameClock() {
        ...
        if (flightState._viewer) flightState._viewer.useDefaultRenderLoop = true;
      },
    }),
  } : {}),
   ...
   */
  getStats() {
    const retryInSec = flightState._retryAt ? Math.max(0, Math.ceil((flightState._retryAt - Date.now()) / 1000)) : 0;
    return {
...
return { _approxDistanceKm, _likelyLanded, _toCleanText, _contactLabel, _describeFlight, _normalizeTrackedIcao, mapAnalystRecord, methods };
}
