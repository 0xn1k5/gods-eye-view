...
import { ENRICH_MAX_INFLIGHT, ENRICH_DISPATCH_GAP_MS, ENRICH_AMBIENT_BUDGET_CEIL, ENRICH_AMBIENT_REFILL_TOKENS, ENRICH_AMBIENT_REFILL_WINDOW_MS, ENRICH_AMBIENT_PER_SWEEP } from './policy.js';

export function createEnrichment({ flightState, services, parts, layer, resolveAsset }) {
...
function _enqueueEnrich(key, query, onData, priority = false) {
  ...
  if (priority) flightState._enrichQueue.unshift(job); else flightState._enrichQueue.push(job);
  _drainEnrich();
}
...
function _drainEnrich() {
  if (flightState.lifetime.signal.aborted) return;
  while (flightState._enrichActive < ENRICH_MAX_INFLIGHT && flightState._enrichQueue.length) {
    ...
    const wait = ENRICH_DISPATCH_GAP_MS - (Date.now() - flightState._enrichLastDispatchMs);
    if (wait > 0) {
      if (!flightState._enrichDripTimer) {
        flightState._enrichDripTimer = setTimeout(() => { flightState._enrichDripTimer = null; _drainEnrich(); }, wait);
      }
      return;
    }
    ...
    Promise.resolve().then(() => {
      ...
      return flightState._source.getEnrichment?.(job.query, { signal: lifetime.signal });
    })
      .then((data) => { if (!lifetime.signal.aborted && data && data.found) job.onData(data); })
      .catch(() => { /* enrichment never surfaces errors */ })
      .finally(() => { if (lifetime.signal.aborted) return; flightState._enrichActive -= 1; _drainEnrich(); });
  }
}
...
function _requestTypeEnrichment(icao24, priority = false) {
  ...
  _enqueueEnrich(`t:${icao24}`, { kind: 'type', id: icao24.toLowerCase() }, (data) => {
    ...
    if (icao24 === flightState._trackedIcao && flightState._trackedEntity) parts.tracking._updateTrackedLabelModel(icao24);
  }, priority);
}
...
function _requestRouteEnrichment(icao24) {
  const cs = String(flightState._flightData.get(icao24)?.callsign || '').trim().toUpperCase();
...
function _refillAmbientBudget(nowMs) {
  ...
  if (!flightState._enrichAmbientRefillAnchorMs) { flightState._enrichAmbientRefillAnchorMs = nowMs; return; }
  const windows = Math.floor((nowMs - flightState._enrichAmbientRefillAnchorMs) / windowMs);
  if (windows <= 0) return;
  flightState._enrichAmbientBudget = Math.min(ceil, flightState._enrichAmbientBudget + windows * refillTokens);
...
function _sweepAmbientEnrichment() {
  ...
  if (flightState._enrichAmbientBudget <= 0 || !flightState._viewer || !flightState._billboardCollection || !flightState._billboardCollection.show) return;
  try {
    ...
    for (const [icao24, bb] of flightState._billboards) {
      ...
      if (cull.computeVisibility(flightState._scratchModelBS) === Cesium.Intersect.OUTSIDE) continue; // off-screen
    ...
    const n = Math.min(cand.length, ENRICH_AMBIENT_PER_SWEEP, flightState._enrichAmbientBudget);
    for (let i = 0; i < n; i++) {
...
return { _enqueueEnrich, _drainEnrich, _requestTypeEnrichment, _requestRouteEnrichment, _ambientBudgetKnobs, _refillAmbientBudget, _sweepAmbientEnrichment };
}
