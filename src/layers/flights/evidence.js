...
export function createEvidence({ flightState, services, parts, layer, resolveAsset }) {
...
function _setFocusEvidenceAircraft(records = []) {
  if (!FOCUS_EVIDENCE_DEV || !flightState._billboardCollection || !flightState._viewer) return { ok: false, count: 0 };
  if (flightState._trackedIcao) parts.tracking._clearTracking();
  parts.rendering._releaseModels();
  for (const bb of flightState._billboards.values()) flightState._billboardCollection.remove(bb);
...
function _focusEvidenceSnapshot() {
  ...
  return [...flightState._focusEvidenceIds].map((id) => {
    ...
    const screen = bb?.position
      ? Cesium.SceneTransforms.worldToWindowCoordinates(flightState._viewer.scene, bb.position)
      : null;
    return {
      id,
      show: bb?.show === true,
      ...
      cameraDistanceM: bb?.position
        ? Cesium.Cartesian3.distance(flightState._viewer.camera.positionWC, bb.position)
        : null,
    };
  });
}
return { _focusEvidencePosition, _setFocusEvidenceAircraft, _moveFocusEvidenceAircraft, _focusEvidenceSnapshot };
}
