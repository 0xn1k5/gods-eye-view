import { CLASS_MODEL_REAL, CLASS_SCALE_3D, CLASS_SCALE_2D } from '../../data/aircraftClass.js';
import { visualCenterForModel, trailAnchorForModel } from '../../data/modelVisualAnchor.js';
...
import { cameraPoseSignature, horizonOccluder, screenProjectedRotation } from '../../data/iconOrientation.js';
import { limitCourseStep, courseSlewCapDps } from '../../data/motionModel.js';
import { PLANE_MODEL_SCALE, PLANE_MODEL_URL, PLANE_NATIVE_RADIUS_M, PLANE_BELLY_OFFSET_NATIVE, MODEL_SCALE, JET_MODEL_URL, MODEL_NATIVE_RADIUS_M, MODEL_BELLY_OFFSET_NATIVE, MODEL_HEADING_OFFSET_DEG, BILLBOARD_SCALE, GROUND_SCALE, COCKPIT_CONTACT_SIZE_PX, MIL_ICON_COLOR, TRACKED_ICON_COLOR, MODEL_ALT_CEIL_M, MODEL_MAX_ALL, MODEL_MAX, COCKPIT_MODEL_MAX, MODEL_ALL_ADD_M, MODEL_PROX_ADD_M, MODEL_ALL_KEEP_M, MODEL_PROX_KEEP_M, IR_RELOAD_BATCH, MODEL_MIN_PX, MODEL_COLOR_BLEND_AMOUNT, TRACKED_MODEL_MIN_PX, TRACKED_MODEL_MAX_PX, FLEET_DR_INTERVAL_MS, COURSE_SLEW_DT_MAX_SEC, ROTATION_REFRESH_MS, COURSE_MAX_DPS } from './policy.js';

export function createRendering({ flightState, services, parts, layer, resolveAsset }) {
const { tr3bIconKind, isTr3b } = services.aircraftPresentation;
const { trackedModelScaleForPixelCap } = services.camera;
const { focusNowMs, getFocusTarget, nearFarScalarValueAtDistance, advanceProjectedSpriteFocus } = services.focus;
const { applyAircraftBillboardTreatment, applyAircraftModelTreatment } = services.recession;
...
function _applyFleetBillboardPresentation(icao24, bb) {
  ...
  const isCockpitContact = flightState._cockpitContactMode && icao24 !== flightState._trackedIcao;
  const isCockpitNear = isCockpitContact && flightState._cockpitNearContacts.has(icao24);
...
const _iconKind = (icao24, klass) => tr3bIconKind(icao24, klass, { hot: flightState._irBoost });
...
function _modelRegimeActive() {
  ...
  const h = flightState._viewer?.camera?.positionCartographic?.height ?? Infinity;
...
function _modelCap() {
  const mapCap = flightState._models3dMode === 'all' ? MODEL_MAX_ALL : MODEL_MAX;
  // `Math.min` on purpose: cockpit may only ever LOWER the GLB budget.
  return flightState._cockpitContactMode ? Math.min(COCKPIT_MODEL_MAX, mapCap) : mapCap;
}
...
function _modelAddDistM() {
  return flightState._models3dMode === 'all' ? MODEL_ALL_ADD_M : MODEL_PROX_ADD_M;
}


function _modelKeepDistM() {
  return flightState._models3dMode === 'all' ? MODEL_ALL_KEEP_M : MODEL_PROX_KEEP_M;
}
...
function _modelMatrix(pos, headingDeg, result = flightState._scratchModelMtx, offsetDeg = MODEL_HEADING_OFFSET_DEG) {
  flightState._scratchModelHpr.heading = Cesium.Math.toRadians((headingDeg || 0) + offsetDeg);
  ...
  return Cesium.Transforms.headingPitchRollToFixedFrame(
    pos, flightState._scratchModelHpr, Cesium.Ellipsoid.WGS84, undefined, result,
  );
}
...
function _modelDisplayPosition(icao24, pos, result) {
  ...
  const h = flightState._groundSnap.heightFor(flightState._viewer, icao24, pos, _groundSampleExclusions);
  if (h == null) return null;
  const carto = Cesium.Cartographic.fromCartesian(pos, Cesium.Ellipsoid.WGS84, flightState._scratchGroundCarto);
...
function _driveFleetModelHandoff(icao24, model, bb, pos, course, beforeShow) {
  ...
  const displayPos = _modelDisplayPosition(icao24, pos, flightState._scratchGroundPos);
  if (!displayPos) {
...
function _modelOwnsVisual(icao24) {
  if (icao24 === flightState._trackedIcao) {
    return parts.tracking._trackedModelRegimeActive() && _modelIsRendering(flightState._trackedModel);
  }
...
function _syncModelToClass(icao24) {
  ...
  if ((current && current._gevSpecKey !== key) || (!current && flightState._modelPending.has(icao24))) {
    const bb = flightState._billboards.get(icao24);
    if (bb && icao24 !== flightState._trackedIcao) bb.show = true;
    _releaseModel(icao24);
  }
  if (icao24 === flightState._trackedIcao && flightState._trackedModel && flightState._trackedModel._gevSpecKey !== key) {
...
function _reloadModelsForIrBoost() {
  ...
  for (const icao of flightState._modelPending) {
    if (!flightState._models.has(icao)) flightState._modelGen.set(icao, (flightState._modelGen.get(icao) || 0) + 1);
  }
  _releaseTrackedModel();
}
...
function _drainIrReloadQueue() {
  ...
  if (flightState._irReloadQueue.length === 0) flightState._irReloadQueue = null;
}
...
async function _ensureModel(icao24) {
  ...
  if (flightState._models.has(icao24) || flightState._modelPending.has(icao24)) return;
  ...
  if ((flightState._models.size + flightState._modelPending.size) >= _modelCap()) return;
  ...
  try {
    ...
    model = await Cesium.Model.fromGltfAsync({
      ...
      customShader: flightState._irBoost ? flightState._IR_UNLIT_SHADER : undefined,
    ...
    if (epoch === flightState._modelEpoch) { flightState._modelPending.delete(icao24); _cleanupModelGen(icao24); }
    return;
  }
  ...
  if (epoch !== flightState._modelEpoch) { try { model.destroy(); } catch { /* gone */ } return; }
  ...
  const stale = (flightState._modelGen.get(icao24) || 0) !== gen
    || !_modelRegimeActive() || !flightState._modelCollection || flightState._modelCollection.isDestroyed()
    || !flightState._flightData.has(icao24) || icao24 === flightState._trackedIcao
    || flightState._models.has(icao24) || flightState._models.size >= _modelCap()
...
function _releaseModel(icao24) {
  ...
  if (m || pending) {
    flightState._modelGen.set(icao24, (flightState._modelGen.get(icao24) || 0) + 1);
  }
  if (m) {
    if (flightState._modelCollection && !flightState._modelCollection.isDestroyed()) { try { flightState._modelCollection.remove(m); } catch { /* gone */ } }
...
function _cleanupModelGen(icao24) {
  if (!flightState._modelPending.has(icao24) && !flightState._models.has(icao24)) flightState._modelGen.delete(icao24);
}
...
function _releaseModels() {
  ...
  for (const icao of flightState._modelPending) flightState._modelGen.set(icao, (flightState._modelGen.get(icao) || 0) + 1);
  if (flightState._modelCollection && !flightState._modelCollection.isDestroyed()) {
    for (const m of flightState._models.values()) { try { flightState._modelCollection.remove(m); } catch { /* gone */ } }
  }
...
function _releaseTrackedModel() {
  ...
  if (flightState._trackedModel) {
    if (flightState._modelCollection && !flightState._modelCollection.isDestroyed()) { try { flightState._modelCollection.remove(flightState._trackedModel); } catch { /* gone */ } }
...
function _updateTrackedModel() {
  const active = flightState._trackedIcao && parts.tracking._trackedModelRegimeActive()
    && flightState._modelCollection && !flightState._modelCollection.isDestroyed();
  if (!active) { if (flightState._trackedModel) flightState._trackedModel.show = false; return; }
  ...
  const pos = parts.motion._trackedDisplayPosition(flightState._trackedIcao) || flightState._billboards.get(flightState._trackedIcao)?.position;
  if (!pos) { if (flightState._trackedModel) flightState._trackedModel.show = false; return; }
  if (!flightState._trackedModel && !flightState._trackedModelLoading && parts.tracking._trackedModelLoadAllowed()) {
    flightState._trackedModelLoading = true;
    const gen = flightState._trackedModelGen;
    const trackedSpec = _modelSpec(flightState._flightData.get(flightState._trackedIcao)?.klass);
    const trackedKey = _specKeyFor(flightState._flightData.get(flightState._trackedIcao)?.klass);
    ...
    Cesium.Model.fromGltfAsync({
      ...
      if (gen !== flightState._trackedModelGen || !flightState._modelCollection || flightState._modelCollection.isDestroyed()) { try { m.destroy(); } catch { /* gone */ } return; }
      ...
      if (_specKeyFor(flightState._flightData.get(flightState._trackedIcao)?.klass) !== trackedKey || flightState._irBoost !== trackedIrBoost) {
      ...
      const currentPos = parts.motion._trackedDisplayCached()
        || flightState._billboards.get(flightState._trackedIcao)?.position;
      if (currentPos) {
        const displayPos = _modelDisplayPosition(flightState._trackedIcao, currentPos, flightState._scratchGroundPos);
        if (displayPos) {
          _modelMatrix(displayPos, parts.motion._trackedDisplayCourse(), m.modelMatrix, trackedSpec.headingOffsetDeg);
        }
      }
  ...
  if (flightState._trackedModel) {
    ...
    const displayPos = _modelDisplayPosition(flightState._trackedIcao, pos, flightState._scratchGroundPos);
    if (!displayPos) {
      flightState._trackedModel.show = false; // no ground evidence → the billboard carries it
      return;
    }
    const spec = _modelSpec(flightState._flightData.get(flightState._trackedIcao)?.klass);
    _modelMatrix(displayPos, parts.motion._trackedDisplayCourse(), flightState._trackedModel.modelMatrix, spec.headingOffsetDeg);
    ...
    flightState._trackedModel.scale = trackedModelScaleForPixelCap({
      ...
      rangeM: Cesium.Cartesian3.distance(flightState._viewer.camera.positionWC, displayPos),
...
function _fleetTick() {
  if (!flightState._viewer || !flightState._billboardCollection || !flightState._billboardCollection.show) return;
  ...
  const tickDtSec = flightState._lastFleetTickMs
    ? Math.min(COURSE_SLEW_DT_MAX_SEC, (nowMs - flightState._lastFleetTickMs) / 1000)
    : 0.08;
  flightState._lastFleetTickMs = nowMs;

  _drainIrReloadQueue(); // bounded per-tick slice of any pending boost-flip reload
  if (flightState._cockpitContactMode) parts.tracking._refreshCockpitNearContacts();
  ...
  const doRotations = (poseSig !== flightState._lastCamPoseSig || (nowMs - flightState._lastRotPassMs) >= ROTATION_REFRESH_MS);
  if (doRotations) {
  ...
  if (!useModels && (flightState._models.size || flightState._modelPending.size)) _releaseModels();
  ...
  if (useModels) {
    ...
    for (const [icao, bb] of flightState._billboards) {
      ...
      Cesium.Cartesian3.clone(bb.position, flightState._scratchModelBS.center);
      cand.push([icao, d2, cull.computeVisibility(flightState._scratchModelBS) !== Cesium.Intersect.OUTSIDE]);
    }
    cand.sort((a, b) => a[1] - b[1]);
    if (cand.length > cap && (nowMs - flightState._lastModelCapWarnMs) > 5000) {
      console.warn(`[Data:Military] ${cand.length} planes in 3D range; capped at ${cap} (${flightState._models3dMode}). On-screen prioritized.`);
      flightState._lastModelCapWarnMs = nowMs;
    }
    modelEligible = new Set();
    for (const [icao, , inF] of cand) { if (modelEligible.size >= cap) break; if (inF && flightState._models.has(icao)) modelEligible.add(icao); } // 1. KEEP on-screen
    ...
    for (const [icao, , inF] of cand) { if (modelEligible.size >= cap) break; if (!inF && flightState._models.has(icao)) modelEligible.add(icao); } // 3. KEEP off-screen (can't starve visible)
    ...
    for (const icao of flightState._models.keys()) {
      if (icao !== flightState._trackedIcao && !modelEligible.has(icao)) toRelease.push(icao);
    }
  ...
  for (const [icao24, bb] of flightState._billboards) {
    ...
    const beyondHorizon = !occluder.isPointVisible(flightState._flightData.get(icao24)?.cullPosition || bb.position);
    ...
    const isCockpitNear = flightState._cockpitContactMode && flightState._cockpitNearContacts.has(icao24);
    const treatment = applyAircraftBillboardTreatment({
      billboard: bb,
      baseScale: flightState._cockpitContactMode && !isCockpitNear ? 1 : _militaryBillboardScale(icao24),
    ...
    if (!flightState._cockpitContactMode || isCockpitNear) {
      ...
      if (wantLarge !== !!bb._gevIconLarge) {
        ...
        bb.image = aircraftIcon(_iconKind(icao24, flightState._flightData.get(icao24)?.klass), wantLarge ? TRACKED_ICON_PX : undefined);
      }
    }
    ...
    const rawCourse = flightState._drCourseDeg != null ? flightState._drCourseDeg : ((info && info.track) || 0);
    const prevCourse = flightState._displayCourse.get(icao24);
    const course = (flightState._drCourseHold && prevCourse != null)
      ? prevCourse
      : limitCourseStep(
        prevCourse, rawCourse,
        courseSlewCapDps(flightState._drSpeedMps != null ? flightState._drSpeedMps : ((info && info.speedMps) ?? NaN), COURSE_MAX_DPS),
        tickDtSec,
      );
      ...
      const ownsVisual = _driveFleetModelHandoff(
        ...
        () => {
          applyAircraftModelTreatment({
            ...
            baseColor: flightState._irBoost ? Cesium.Color.WHITE : _modelColor(icao24),
    ...
    if ((!flightState._cockpitContactMode || isCockpitNear) && (doRotations || revealed)) {
...
return { _groundDepthDistance, _modelSpec, _normalBillboardScaleByDistance, _cockpitBillboardScaleByDistance, _militaryBillboardScale, _applyFleetBillboardPresentation, _iconKind, _modelColor, _modelRegimeActive, _modelCap, _modelAddDistM, _modelKeepDistM, _modelMatrix, _groundSampleExclusions, _modelDisplayPosition, _driveFleetModelHandoff, _modelOwnsVisual, _modelIsRendering, _specKeyFor, _syncModelToClass, _reloadModelsForIrBoost, _drainIrReloadQueue, _ensureModel, _releaseModel, _cleanupModelGen, _releaseModels, _releaseTrackedModel, _updateTrackedModel, _fleetTick };
}
