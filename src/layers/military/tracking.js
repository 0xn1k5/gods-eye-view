...
import { bindTrackingClickGesture, isTrackingSelectionGesture, isTrackingClickGesture } from '../../data/trackingClickGesture.js';
import { TRACKED_MODEL_MAX_LOAD_FAILS, TRACKED_MODEL_RETRY_BACKOFF_MS, TRAIL_MAX_POINTS, RENDER_DELAY_SEC, TRAIL_COLOR, BILLBOARD_SCALE, GROUND_SCALE, MIL_ICON_COLOR, AMBER_TRANSPARENT, TRACKED_ICON_COLOR } from './policy.js';

export function createTracking({ flightState, services, parts, layer, resolveAsset }) {
const { selectTrackedSubjectContext, refreshTrackedSubjectContext, clearTrackedSubjectContext } = services.context;
const { tr3bTypeLabel, isTr3b, tr3bConvertedIds } = services.aircraftPresentation;
const { trackedLabelModelFromText, refreshTrackedReadout } = services.readout;
const { floorAltitudeM, cachedGroundFloor, resolveGroundFloorCellsBounded } = services.groundFloor;
...
function _publishTrackedSelection(icao24, origin = 'programmatic') {
  ...
  if (flightState._trackedEntity) flightState._trackedEntity.gevSelectionOrigin = origin;
  _emitAwarenessEvent('gev:awareness-subject-selected', {
    layerId: 'military',
    id: icao24,
    label: parts.queries._toCleanText(info.callsign) || parts.queries._toCleanText(info.registration) || icao24,
...
function _contextSubjectMetadata(icao24) {
  ...
  return {
    ...
    properties: {
      ...
      type: tr3bTypeLabel(icao24, parts.queries._toCleanText(info?.type) || ''),
      altitude: described.onGround ? 'on ground' : parts.queries._formatAltitude(info?.altitudeFt),
...
function _refreshCockpitNearContacts() {
  if (!flightState._cockpitContactMode || !flightState._viewer?.camera?.positionWC) {
    if (flightState._cockpitNearContacts.size) flightState._cockpitNearContacts = new Set();
    return;
  }
  ...
  for (const [icao24, bb] of flightState._billboards) {
    ...
    distancesSquared.push([
      icao24,
      Cesium.Cartesian3.distanceSquared(flightState._viewer.camera.positionWC, bb.position),
    ]);
  }
...
function _setCockpitContactMode(active) {
  ...
  for (const [icao24, bb] of flightState._billboards) parts.rendering._applyFleetBillboardPresentation(icao24, bb);
...
function _buildTrackedLabel(info, icao24) {
  const stale = (flightState._missingPolls.get(icao24) || flightState._backoff) ? ' · STALE' : '';
  const callsign = (parts.queries._toCleanText(info?.callsign) || parts.queries._toCleanText(info?.registration) || icao24) + stale;
  // Converted contacts report their class as TR-3B — that override is exactly
  // what the Easter egg replaces the real type with.
  const type = tr3bTypeLabel(icao24, parts.queries._toCleanText(info?.type) || 'Type unknown');
  const registration = parts.queries._toCleanText(info?.registration) || 'Reg unknown';
  const operator = parts.queries._toCleanText(info?.operator) || 'Operator unknown';
...
function _updateTrackedLabelModel(icao24) {
  if (!flightState._trackedEntity || icao24 !== flightState._trackedIcao) return;
...
function _syncTrackedBillboardImage() {
  if (!flightState._trackedIcao || !flightState._trackedEntity?.billboard) return;
  flightState._trackedEntity.billboard.image = aircraftIcon(
    parts.rendering._iconKind(flightState._trackedIcao, flightState._flightData.get(flightState._trackedIcao)?.klass),
    TRACKED_ICON_PX,
  );
}
...
function _refreshTr3bContact(icao24) {
  ...
  if (!id) return false;
  if (isTr3b(id)) {
    if (flightState._models.has(id) || flightState._modelPending.has(id)) parts.rendering._releaseModel(id);
...
function _refreshTr3bForStyle() {
  for (const id of tr3bConvertedIds()) {
    if (flightState._billboards.has(id) || id === flightState._trackedIcao) _refreshTr3bContact(id);
  }
}
...
function _trackedModelLoadAllowed(nowMs = Date.now()) {
  if (flightState._trackedModelFailIcao !== flightState._trackedIcao) return true; // untried selection
  if (flightState._trackedModelFailCount >= TRACKED_MODEL_MAX_LOAD_FAILS) return false;
...
function _noteTrackedModelLoadFailure(url, err) {
  ...
  flightState._trackedModelRetryAtMs = Date.now() + TRACKED_MODEL_RETRY_BACKOFF_MS;
...
function _trackedModelRegimeActive() {
  ...
  if (!flightState._trackedIcao || flightState._cockpitContactMode || isTr3b(flightState._trackedIcao)) {
    flightState._trackedZoomLatched = false;
    return false;
  }
...
function _syncTracked2dRotation() {
  ...
  const pos = parts.motion._trackedDisplayCached() || flightState._billboards.get(flightState._trackedIcao)?.position;
  if (!pos) return;
  ...
  const rotation = stabilizeScreenRotation(flightState._lastTrackedRotation, projected, 0);
...
function _trailFloorPosition(position) {
  const carto = Cesium.Cartographic.fromCartesian(position, Cesium.Ellipsoid.WGS84, flightState._scratchTrailCarto);
...
function _appendTrailFix(position) {
  ...
  if (flightState._trailPositions.length > TRAIL_MAX_POINTS) flightState._trailPositions.shift();
...
function _refreshTrailDisplay() {
  ...
  flightState._trail.setPositions(flightState._trailPositions.length > 1 ? flightState._trailPositions.slice(0, -1) : flightState._trailPositions);
}
...
function _startTrail(icao24) {
  ...
  const seedRenderTime = Cesium.JulianDate.addSeconds(
    Cesium.JulianDate.now(), -RENDER_DELAY_SEC, flightState._scratchWarmupTime
  );
  for (const fix of history) {
    if (Cesium.JulianDate.lessThanOrEquals(fix.time, seedRenderTime)) {
      ...
      flightState._trailPositions.push(_trailFloorPosition(Cesium.Cartesian3.clone(fix.position)));
    }
  }
  if (!flightState._trail && flightState._viewer) {
    flightState._trail = createTrail(flightState._viewer, { color: TRAIL_COLOR, width: 2.5 });
  }
    ...
    flightState._trailHeadEntity = flightState._viewer.entities.add({
      ...
      polyline: {
        positions: new Cesium.CallbackProperty(() => {
          ...
          if (!flightState._trackedIcao || flightState._trailPositions.length < 2 || parts.motion._isTrackWarmingUp()) return [];
          const head = parts.motion._trackedTrailCached() || parts.motion._trackedDisplayPosition(flightState._trackedIcao);
          if (!head) return [];
          // body[n−2] (last displayed body point) → delayed head: runs FORWARD, never a
          // backward/reversing segment.
          const start = flightState._trailPositions[flightState._trailPositions.length - 2];
          ...
          const from = trailHeadStart(
            start, end, parts.motion._trackedModelCenterWorld(), parts.motion._trackedModelEnvelopeM(), flightState._scratchTrailHead,
          );
          if (!from) return [];
          return [from, end];
        }, false),
...
async function _backfillTrail(icao24, token, oldestFixEpochSec) {
  let trace = null;
  try {
    const track = await flightState._source.getTrack?.(flightState._flightData.get(icao24)?.sourceReference ?? icao24, { signal: AbortSignal.any([flightState.lifetime.signal, AbortSignal.timeout(8000)]) });
    trace = track?.records ?? null;
  } catch {
    return; // silent fallback to the accumulated trail
  }
  if (!trace) return;
  if (token !== flightState._trailBackfillToken || icao24 !== flightState._trackedIcao) return;
  ...
  const budget = Math.max(1, TRAIL_MAX_POINTS - flightState._trailPositions.length);
  ...
  if (flightState._trailPositions.length > TRAIL_MAX_POINTS) {
    flightState._trailPositions = flightState._trailPositions.slice(flightState._trailPositions.length - TRAIL_MAX_POINTS);
  }
  _refreshTrailDisplay();
}
...
function _clearTrail() {
  ...
  if (flightState._trailHeadEntity && flightState._viewer && !flightState._viewer.isDestroyed()) {
    try { flightState._viewer.entities.remove(flightState._trailHeadEntity); } catch { /* already gone */ }
  }
...
function _clearTracking(skipViewerUntrack = false, {
  ...
  parts.rendering._applyFleetBillboardPresentation(clearedIcao, flightState._billboards.get(clearedIcao));
...
function _applyPendingTrackingRestore() {
  ...
  if (!pending || pending.generation !== flightState._trackingIntentGeneration) return false;
  if (!flightState._billboardCollection?.show || !flightState._billboards.has(pending.id)) return false;
...
function _trackFlight(icao24, { origin = 'programmatic' } = {}) {
  ...
  // aircraft has no fix.
  const getTrackedPosition = () => parts.motion._trackedDisplayPosition(icao24) || bb.position;
  ...
  flightState._trackedEntity = flightState._viewer.entities.add({
    ...
    billboard: {
      image: aircraftIcon(parts.rendering._iconKind(flightState._trackedIcao, flightState._flightData.get(flightState._trackedIcao)?.klass), TRACKED_ICON_PX),
      width: 28,
      height: 28,
      scale: BILLBOARD_SCALE * (CLASS_SCALE_2D[flightState._flightData.get(flightState._trackedIcao)?.klass] || 1),
      ...
      color: new Cesium.CallbackProperty(() => (
        parts.rendering._modelOwnsVisual(flightState._trackedIcao) ? AMBER_TRANSPARENT : TRACKED_ICON_COLOR
      ), false),
      sizeInMeters: false,
      ...
      rotation: new Cesium.CallbackProperty(() => {
        ...
        if (!tracked || !pos || !flightState._viewer) return flightState._lastTrackedRotation;
        const projected = screenProjectedRotation(
          flightState._viewer.scene,
          pos,
          parts.motion._trackedDisplayCourse(),
        ...
        const rot = stabilizeScreenRotation(flightState._lastTrackedRotation, projected);
  ...
  flightState._trackedEntity.viewFrom = new Cesium.Cartesian3(0, -followRange * 0.8, followRange * 0.55);
  ...
  flightState._trackedEntity.gevDisplayPosition = parts.motion._trackedDisplayCached;
  ...
  flightState._trackedEntity.gevVisualPosition = parts.motion._trackedVisualCached;
  refreshTrackedReadout(flightState._trackedEntity);
  ...
  flightState._trackedCameraFrameStop = applyTrackedCameraFrame(
    flightState._viewer,
  ...
  const callsign = parts.queries._toCleanText(info.callsign) || parts.queries._toCleanText(info.registration) || icao24;
...
function _installClickHandler(viewer) {
  ...
  if (!flightState._trackedEntityChangedRemove) {
    flightState._trackedEntityChangedRemove = viewer.trackedEntityChanged.addEventListener(() => {
      if (flightState._trackedIcao && flightState._viewer && flightState._viewer.trackedEntity && flightState._viewer.trackedEntity !== flightState._trackedEntity) {
        _clearTracking(true, {
          origin: flightState._viewer.trackedEntity?.gevSelectionOrigin || 'programmatic',
        });
      }
    });
  }

  flightState._clickHandler = new Cesium.ScreenSpaceEventHandler(viewer.scene.canvas);
  bindTrackingClickGesture(flightState._clickHandler, (click, gesture) => {
    ...
    if (picked) {
      ...
      if (flightState._trackedIcao) {
        ...
        if (picked.primitive === flightState._trackedModel || rawPick === flightState._trackedIcao) return;
      }
      ...
      if (billboard && billboard.id && flightState._billboards.has(billboard.id)) {
        _cancelPendingTrackingRestore();
        _trackFlight(billboard.id, { origin: 'user' });
        return;
      }
      // Fallback: some CesiumJS versions surface the id string at picked.id
      if (picked.id && typeof picked.id === 'string' && flightState._billboards.has(picked.id)) {
        _cancelPendingTrackingRestore();
...
return { _emitAwarenessEvent, _publishTrackedSelection, _contextSubjectMetadata, _isExplicitTrackingOrigin, _refreshCockpitNearContacts, _setCockpitContactMode, _applyCockpitState, _buildTrackedLabel, _updateTrackedLabelModel, _syncTrackedBillboardImage, _refreshTr3bContact, _refreshTr3bForStyle, _resetTrackedSelectionState, _trackedModelLoadAllowed, _noteTrackedModelLoadFailure, _trackedModelRegimeActive, _syncTracked2dRotation, _trailFloorPosition, _appendTrailFix, _refreshTrailDisplay, _startTrail, _backfillTrail, _clearTrail, _destroyTrail, _clearTracking, _applyPendingTrackingRestore, _cancelPendingTrackingRestore, _trackFlight, _onKeyDown, _installClickHandler };
}
