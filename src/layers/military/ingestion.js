...
import { GROUND_FLOOR_WARM_MAX_ALT_M, POSITION_HISTORY_LIMIT, BILLBOARD_SCALE, GROUND_SCALE, TRACKED_ICON_COLOR, MIL_ICON_COLOR, LANDED_MISSING_POLL_LIMIT, MISSING_POLL_LIMIT, ERROR_BACKOFF_INTERVAL } from './policy.js';

export function createIngestion({ flightState, services, parts, layer, resolveAsset }) {
const { geoidHeight } = services.geoid;
const { floorAltitudeM, cachedGroundFloor, warmGroundFloor, GROUND_FLOOR_LIFT_M } = services.groundFloor;
...
const methods = {
  ...
  async update(viewer, { signal = null } = {}) {
    ...
    try {
      ...
      const snapshot = await flightState._source.getSnapshot({}, { signal: updateSignal });
      updateSignal.throwIfAborted();
      ...
      flightState._backoff = snapshot.stale || snapshot.freshness === 'unknown';
      flightState._retryAt = 0;
      flightState._lastError = snapshot.reason || (snapshot.freshness === 'unknown' ? 'Source snapshot time unavailable' : null);
      ...
      {
        const viewerCarto = flightState._viewer?.camera?.positionCartographic || null;
