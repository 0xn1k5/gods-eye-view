...
function configureSource(nextSource, { signal } = {}) {
  if (typeof nextSource?.getSnapshot !== 'function') throw new TypeError('A snapshot source is required');
  releaseSource?.();
...
async function refreshMilitaryRegistryIfStale() {
  if (!currentSource || !lifetime || lifetime.signal.aborted || _militaryLayerActive) return;
...
return { configureSource, dispose, isMilitaryLayerActive, setMilitaryLayerActive,
  onMilitaryLayerActiveChange, registerMilitaryIcaos, isMilitaryIcao, refreshMilitaryRegistryIfStale };
}
