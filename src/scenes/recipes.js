...
const BHOTE_KOSHI_NEPAL_BOOTSTRAP_RECIPE = BHOTE_KOSHI_NEPAL_APPEND_RECIPE
  ? Object.freeze({
    ...
    cameraPath: BHOTE_KOSHI_NEPAL_APPEND_RECIPE.legacySceneBootstrap.cameraPath
      .slice(0, 3)
      .map((keyframe, index) => ({ ...keyframe, title: `Shot ${index + 1}` })),
  })
  : null;

const EVENT_RECIPES = [
  BHOTE_KOSHI_NEPAL_BOOTSTRAP_RECIPE,
].filter(Boolean);
...
export function createSceneRecipes({
  localDemoRecipes = EVENT_RECIPES,
} = {}) {
