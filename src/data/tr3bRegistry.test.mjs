...
test('both flight layers keep a converted contact 2D and visible (render invariants)', async () => {
  for (const name of ['flights.js', 'militaryFlights.js']) {
    ...
    assert.match(source, /if \(!(?:flightState\.)?_trackedIcao \|\| (?:flightState\.)?_cockpitContactMode \|\| isTr3b\((?:flightState\.)?_trackedIcao\)\) \{/,
    ...
    assert.match(source, /if \(bb && id !== (?:flightState\.)?_trackedIcao\) bb\.show = true;|if \(modelled && id !== (?:flightState\.)?_trackedIcao\) modelled\.show = true;/,
    ...
    for (const call of callSites) {
      assert.match(call, /aircraftIcon\(\s*(?:parts\.rendering\.)?_iconKind\(/,
...
test('a converted contact never consumes a 3D model CAP SLOT', async () => {
  ...
  for (const name of ['flights.js', 'militaryFlights.js']) {
    ...
    const loop = /const cand = \[\];\s*\n\s*for \(const \[icao, bb\] of (?:flightState\.)?_billboards\)[\s\S]*?cand\.push\(/.exec(source)?.[0];
