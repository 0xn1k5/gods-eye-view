...
export const SOURCE_PATH_BEAT_IDS = Object.freeze([
  'debris-dammed-lake', 'second-landslide', 'dhunche',
  'mailung-upper-trishuli', 'mailung-bazzar', 'dandagaun',
  'dandagaun-viewpoint', 'betrawati-bazaar', 'bhainse', 'bidur-trishuli-bridge',
]);

function nearestVertex(point, route) {
  if (!point || !Number.isFinite(point.lon) || !Number.isFinite(point.lat)) return -1;
  ...
  route.forEach(([lon, lat], index) => {
    const metres = Math.hypot((lon - point.lon) * longitudeScale, lat - point.lat) * 111195;
    if (metres < distance) { distance = metres; nearest = index; }
  });
  return nearest;
}
...
export function buildSourceShotPaths(event, route = BHOTE_KOSHI_FLOOD_PATH) {
  const observations = new Map((event.evidenceSpine || []).map((point) => [point.id, point]));
  ...
  const add = (id, from, to, { joinComparison = false } = {}) => {
    ...
    if (joinComparison) {
      ...
      const along = (from.lon - lon) * (nextLon - lon) * scale * scale
        + (from.lat - lat) * (nextLat - lat);
      if (along > 0) start++;
    }
    const coordinates = route.slice(start, end + 1);
    if (joinComparison && (coordinates[0][0] !== from.lon || coordinates[0][1] !== from.lat)) {
      coordinates.unshift([from.lon, from.lat]);
    }
    result.set(id, coordinates.map(([lon, lat], index) => ({
      lon, lat,
      ...
      fallbackElevationM: startHeight + (endHeight - startHeight) * index / (coordinates.length - 1),
    })));
  };
  add('debris-dammed-lake', observations.get('debris-dammed-lake'), observations.get('second-landslide'));
  add('second-landslide', observations.get('second-landslide'), observations.get('gyirong-border-gate'));
